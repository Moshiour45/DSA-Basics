...
// Maximum sum of distances in donward direction
void dfs1(ll node, ll parent, vector<vector<ll>>& adj, vector<ll>& in, vector<ll>& subtree){
    subtree[node] = 1;
    for(auto& child : adj[node]){
        if(parent == child){
            continue;
        }
        dfs1(child, node, adj, in, subtree);
...
// Re-rooting part
void dfs2(int n, ll node, ll parent, vector<vector<ll>>& adj, vector<ll>& in, vector<ll>& dp, vector<ll>& subtree){
    for(auto child : adj[node]){
        if(parent == child){
            continue;
        }
        ...
        dfs2(n, child, node, adj, in, dp, subtree);
    }
}
...
int main(){
    ...
    vector<ll> subtree(n + 1, 0); // Total Nodes in the subtree of a particular node

    dfs1(1, 0, adj, in, subtree);
    dp[1] = in[1];
    dfs2(n, 1, 0, adj, in, dp, subtree);
    for(int i = 1; i <= n; i++){
        cout << dp[i] << " ";
    }
    cout << endl;
